...
def estimate_noise_mad(x):
    ...
    return torch.cat(noise_maps, dim=0), torch.cat(sigmas, dim=0)
...
class MultiScaleFrequencyModulation(nn.Module):
    ...
    def _freq_split(self, x):
        ...
        def _back(f):
            return torch.fft.irfft2(torch.fft.ifftshift(f, dim=-2), s=(H,W), norm='ortho')
...
class EnhancedFeatureBackbone(nn.Module):
    def __init__(self, in_channels=1, hidden_dim=160, drop_path_rate=0.1):
        ...
        self.cnn_blocks = nn.ModuleList([self._res_conv(hidden_dim) for _ in range(6)])
        dpr = [drop_path_rate * i / 3 for i in range(4)]
        self.swin_blocks = nn.ModuleList([
            SwinTransformerBlock(
                dim=hidden_dim, num_heads=8, window_size=window_size,
...
class NAFDMNet(nn.Module):
    def __init__(self, in_channels=1, hidden_dim=160, latent_dim=320):
        super().__init__()
        self.backbone = EnhancedFeatureBackbone(in_channels, hidden_dim)
        self.freq_mod = MultiScaleFrequencyModulation(hidden_dim, latent_dim)
        self.decoder  = SkipDecoder(hidden_dim)
    ...
    def _init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                if m.bias is not None:   init.constant_(m.bias, 0)
        init.constant_(self.decoder.dec2[-1].weight,0)
        init.constant_(self.decoder.dec2[-1].bias, 0)

    def forward(self, noisy, gt=None):
        noise_map, z_sigma = estimate_noise_mad(noisy)
        feat, feat_shallow = self.backbone(noisy)
        mod_feat, freq_weights = self.freq_mod(feat, noise_map, z_sigma)
        residual = self.decoder(mod_feat, feat_shallow)

        # ★ Diagnostic output (printed only once)
        if not self._debug_printed:
            self._debug_printed = True
            print(f"  [Model Debug] noise_map:  [{noise_map.min():.4f},{noise_map.max():.4f}]")
            print(f"  [Model Debug] z_sigma:    [{z_sigma.min():.4f},{z_sigma.max():.4f}]")
            print(f"  [Model Debug] feat:       [{feat.min():.4f},{feat.max():.4f}]  nan:{feat.isnan().any().item()}")
            print(f"  [Model Debug] mod_feat:   [{mod_feat.min():.4f},{mod_feat.max():.4f}]  nan:{mod_feat.isnan().any().item()}")
            print(f"  [Model Debug] residual:   [{residual.min():.4f},{residual.max():.4f}]  nan:{residual.isnan().any().item()}")
            print(f"  [Model Debug] noisy:      [{noisy.min():.4f},{noisy.max():.4f}]")
            output_pre = noisy + residual
            print(f"  [Model Debug] noisy+res:  [{output_pre.min():.4f},{output_pre.max():.4f}]")

        output = torch.clamp(noisy + residual, 0, 1)
        intermediates = {
            'noise_map'   : noise_map.detach(),
...
class SSIMLoss(nn.Module):
    ...
    def forward(self, pred, target):
        w   = self.window.to(pred.device).expand(pred.shape[1], 1, -1, -1)
        pad = self.ws // 2
        mu1 = F.conv2d(pred,   w, padding=pad, groups=pred.shape[1])
        mu2 = F.conv2d(target, w, padding=pad, groups=pred.shape[1])
        mu1_sq, mu2_sq, mu12 = mu1**2, mu2**2, mu1*mu2
        s1  = F.conv2d(pred*pred,     w, padding=pad, groups=pred.shape[1]) - mu1_sq
        s2  = F.conv2d(target*target, w, padding=pad, groups=pred.shape[1]) - mu2_sq
        s12 = F.conv2d(pred*target,   w, padding=pad, groups=pred.shape[1]) - mu12
...
class FrequencyLoss(nn.Module):
    def forward(self, pred, target):
        ...
        return F.l1_loss(pm, tm) + 0.1 * F.l1_loss(torch.angle(pf), torch.angle(tf))
...
class EdgeLoss(nn.Module):
    ...
    def _grad(self, x):
        gx = F.conv2d(x, self.sx.to(x.device), padding=1)
        gy = F.conv2d(x, self.sy.to(x.device), padding=1)
...
class NAFDMLoss(nn.Module):
    def __init__(self, lambda_char=1.0, lambda_ssim=0.1, lambda_freq=0.1,
                 lambda_edge=0.05, lambda_lpips=0.05, use_lpips=True):
        super().__init__()
        self.lw = dict(char=lambda_char, ssim=lambda_ssim,
                       freq=lambda_freq, edge=lambda_edge, lpips=lambda_lpips)
        ...
        if use_lpips:
            try:
                ...
                self.use_lpips = True
                print("[Loss] LPIPS added to backprop with lambda=0.05")
            except ImportError:
                print("[Loss] lpips not installed, skipping")
    ...
    def forward(self, pred, target):
        ...
        total = (self.lw['char']*char + self.lw['ssim']*ssim +
                 self.lw['freq']*freq + self.lw['edge']*edge)
        ld = {'char': char.item(), 'ssim': ssim.item(),
              'freq': freq.item(), 'edge': edge.item(),
              'lpips': 0.0, 'total': 0.0}
        if self.use_lpips:
            p  = (pred*2-1).repeat(1,3,1,1) if pred.shape[1]==1 else pred*2-1
            t  = (target*2-1).repeat(1,3,1,1) if target.shape[1]==1 else target*2-1
            self.lpips_fn = self.lpips_fn.to(pred.device)
            lp = self.lpips_fn(p, t).mean()
            total = total + self.lw['lpips'] * lp
            ld['lpips'] = lp.item()
        ld['total'] = total.item()
        return total, ld
...
if __name__ == '__main__':
    ...
    model = NAFDMNet(hidden_dim=160, latent_dim=320).to(device)
    print(f"Params: {count_params(model)/1e6:.2f}M")
    x  = torch.rand(2, 1, 256, 256).to(device)
    gt = torch.rand(2, 1, 256, 256).to(device)
    with torch.no_grad():
        out, inter = model(x)
        print(f"Output: {out.shape}, range [{out.min():.4f},{out.max():.4f}]")
        print(f"freq_weights: {inter['freq_weights'].view(2,3).cpu().numpy()}")
    criterion = NAFDMLoss().to(device)
    loss, ld  = criterion(out, gt)
    print(f"Loss: {loss.item():.4f}, breakdown: {ld}")